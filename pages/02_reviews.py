...
with tab_edit:
    ...
    if not varieties:
        ...
        with st.form("review_form"):
            variety_id = st.selectbox("品種*", [v["id"] for v in varieties], format_func=lambda x: next(v["name"] for v in varieties if v["id"] == x))
            ...
            uploaded_files = st.file_uploader("画像アップロード (最大3枚)", type=["jpg", "jpeg", "png", "webp"], accept_multiple_files=True)
...
with tab_history:
    varieties = list_active_varieties()
    variety_filter = st.selectbox("品種フィルタ", [""] + [v["id"] for v in varieties], format_func=lambda x: "すべて" if not x else next(v["name"] for v in varieties if v["id"] == x))
    ...
    rows, total = list_reviews(variety_id=variety_filter or None, date_from=date_from, date_to=date_to, page=page, page_size=page_size)
    st.caption(f"合計: {total}件")
    render_table(rows)
    delete_id = st.selectbox("削除対象", [""] + [r["id"] for r in rows], format_func=lambda x: next((f"{r['tasted_date']} {r['id'][:8]}" for r in rows if r["id"] == x), ""))
...
with tab_deleted:
    ...
    restore_id = st.selectbox("復元対象", [""] + [r["id"] for r in deleted_rows], format_func=lambda x: next((f"{r['tasted_date']} {r['id'][:8]}" for r in deleted_rows if r["id"] == x), ""))
