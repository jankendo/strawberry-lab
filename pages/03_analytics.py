...
from src.services.analytics_service import get_filtered_review_dataframe, monthly_timeseries, prefecture_counts, radar_data, ranking_data, scatter_data
...
selected_varieties = st.multiselect("対象品種", [v["id"] for v in varieties], format_func=lambda x: next((v["name"] for v in varieties if v["id"] == x), x))

df = get_filtered_review_dataframe(date_from=date_from, date_to=date_to, prefecture=prefecture or None, tags=tags, variety_ids=selected_varieties or None)
if df.empty:
    st.info("データがありません。")
    st.stop()

st.subheader("A. レーダーチャート")
radar_df = radar_data(df, min_count, selected_varieties or None)
if not radar_df.empty:
    radar_long = radar_df.melt(id_vars=["variety_id", "variety_name", "review_count"], value_vars=["sweetness", "sourness", "aroma", "texture", "appearance"], var_name="metric", value_name="value")
    fig_radar = px.line_polar(radar_long, r="value", theta="metric", color="variety_name", line_close=True)
    st.plotly_chart(fig_radar, use_container_width=True)

st.subheader("B. 総合評価ランキング")
ranking_rows = ranking_data(df, min_count)
if ranking_rows:
    st.plotly_chart(px.bar(ranking_rows, x="name", y="avg_overall", hover_data=["review_count"]), use_container_width=True)
...
st.subheader("D. 糖度と総合評価")
scatter_rows = scatter_data(df)
if scatter_rows:
    st.plotly_chart(
        px.scatter(scatter_rows, x="brix_midpoint", y="avg_overall", size="review_count", hover_name="name"),
        use_container_width=True,
    )

st.subheader("E. 都道府県マップ")
...
if geo_path.exists():
    ...
    if not map_df.empty and geojson.get("features"):
        fig_map = px.choropleth(map_df, geojson=geojson, featureidkey="properties.name", locations="prefecture", color="count")
...
st.download_button("分析用ベースデータCSV", data=export_table_csv("reviews"), file_name="reviews_export.csv", mime="text/csv")
