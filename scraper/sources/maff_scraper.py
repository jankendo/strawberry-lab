...
class MaffScraper(BaseScraper):
    """Scraper for MAFF pages."""

    def fetch_article_links(self) -> list[str]:
        links: list[str] = []
        for listing_url in self.source_config.listing_urls:
            response = self._get(listing_url)
            links.extend(self.parse_links_from_html(response.text, "https://www.maff.go.jp"))
        return [link for link in links if "maff.go.jp" in link][: self.source_config.max_articles_per_run]
