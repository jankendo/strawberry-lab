...
def test_native_shell_bootstrap_caches_static_base_and_uses_direct_nav_links(monkeypatch) -> None:
    ...
    assert "ensureAuthCookieFromStorage" in captured["html"]
    assert "__sl_remember_auth_v1" in captured["html"]
    assert "installAuthCookieRecovery();" in captured["html"]
    ...
    assert 'ensureAuthCookieFromStorage({ force: true });' in captured["html"]
